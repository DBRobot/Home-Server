//! `dd media`: every library this machine can open, as folders on this
...
use crate::librarycmd::{self, Gate, Opener};
...
/// mount every openable library under `at`, then run the player if asked
pub async fn run(
    keys: &auth::Store,
    dirs: &[String],
    at: Option<PathBuf>,
    ...
    let base = librarycmd::files_base(dirs)?;
    ...
    for (owner, lib, key) in librarycmd::openable(dirs, &user, &opener).await? {
    ...
    let mountpoint = at.clone();
    let session = fuser::spawn_mount(fs, &mountpoint, &config).with_context(|| {
        format!(
    ...
    let player: Option<std::process::Child> = match jellyfin {
        Some(bin) => {
            ...
            println!(
                "jellyfin started with its own data in {} - add {} as a library at http://localhost:8096",
                data.display(),
                mountpoint.display()
            );
            Some(child)
